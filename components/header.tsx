          ...
          >
            <div className="text-2xl md:text-4xl font-display font-black tracking-tighter uppercase focus-visible:outline-none transition-all duration-300 hover:scale-105">
              <span className="relative inline-block">
                <span className="relative z-10 bg-gradient-to-r from-text via-accent to-text bg-clip-text text-transparent group-hover:from-accent group-hover:via-text group-hover:to-accent transition-all duration-500">
                  NINETY NINE
                </span>
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-10">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="text-base lg:text-lg font-display font-bold text-textDim hover:text-text transition-all duration-300 relative py-2 focus-visible:outline-none focus-visible:text-accent group uppercase tracking-tight"
              >
          ...
          <div className="md:hidden pb-6 space-y-3 border-t-2 border-accent/20 pt-4">
            {navigation.map((item) => (
              <Link
                key={item.name}
                href={item.href}
                className="block text-lg font-display font-bold text-textDim hover:text-accent transition-all duration-300 px-4 py-3 rounded-xl hover:bg-accent/10 focus-visible:outline-none focus-visible:bg-accent/10 focus-visible:text-accent uppercase tracking-tight border-2 border-transparent hover:border-accent/30"
